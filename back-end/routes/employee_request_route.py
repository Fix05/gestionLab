...
def get_all_requests_info(id: int, db: mysql.connector.MySQLConnection = Depends(get_db)):
    ...
    try:
        ...
        document_request_query = f""" 
        SELECT path_document_request as path, name_document_request as name
        ...
        print(result)
        if result['response']:
            cursor.execute(responder_request_query, (result['rh'], ))
