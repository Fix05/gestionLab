...
def get_manager_assigned_requests(id: int, roles: 
                                  List[str] = Query(['User'], description='List of roles'), 
    ...
    try:
        ...
        query = f"""
            ...
            type_request as type, reason_request as reason, state_request as state, 
            date_request as date, id_request as id
            FROM requests
...
def get_all_employee_requests_info(id: int, 
                                   db: mysql.connector.MySQLConnection = Depends(get_db)):
    try:
        cursor = db.cursor(dictionary=True)
        infoQuery = """
            SELECT type_request as type, date_request as date, reason_request 
            as reason, explanation_request as explanation, 
            state_request as state, name_document_request as doc, 
            body_response_request as response, name_person as name, 
            lastname_person as lastname, date_response_request as date_response
            FROM requests
...
def get_request_document(id: int, 
                         db: mysql.connector.MySQLConnection = Depends(get_db)):
    try:
        cursor = db.cursor(dictionary=True)
        query = f"""
            SELECT path_document_request as file_path, name_document_request 
            as file_name
        ...
        cursor.close()
        if path:
            return FileResponse(path=f"{path['file_path']}", 
...
def set_request_response(response: Response, id: int, 
                         db: mysql.connector.MySQLConnection = Depends(get_db)):
    try:
        cursor = db.cursor(dictionary=True)
        query = """
            INSERT INTO response_request (body_response_request, 
            date_response_request, fk_request, fk_rh_employee)
            VALUES (%s, %s, %s, %s);
        """
...
def update_request_state(updated_data: dict, id: int, 
                         db: mysql.connector.MySQLConnection = Depends(get_db)):
    try:
        cursor = db.cursor(dictionary=True)
        query = """
